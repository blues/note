...
// you NEED to add it to the end of the list (before the "") because the numbers are stored persistently
// in the tower.cdb database (and only that database).
var tznames = [...]string{
	"Unknown",
	"Africa/Abidjan",
	"Africa/Accra",
...
func TimeZoneNumberToCountry(num int) string {
	...
	components := strings.Split(tznames[num], "/")
	if len(components) < 2 {
		return "Unknown"
	}
	return components[0]
